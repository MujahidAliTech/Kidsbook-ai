...
export const MyBooksView: React.FC<Props> = ({
  ...
  if (books.length === 0) {
    return (
      <div className="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-slate-200 shadow-sm space-y-4 max-w-xl mx-auto my-8">
        <div className="w-16 h-16 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center mx-auto text-3xl">
          <Bookmark className="w-8 h-8" />
        </div>
        <h3 className="text-xl font-extrabold text-slate-900">No Saved Books Yet</h3>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-md border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
        ...
        {books.map((b) => (
          ...
          >
            <div>
              ...
              <h3 className="text-xl font-extrabold text-slate-900 leading-tight">{b.title}</h3>

              <div className="flex flex-wrap gap-2 mt-3">
                <span className="px-2 py-0.5 bg-slate-100 text-slate-700 text-[11px] font-semibold rounded-md">
                  Age {b.ageGroup}
                </span>
                <span className="px-2 py-0.5 bg-slate-100 text-slate-700 text-[11px] font-semibold rounded-md capitalize">
                  {b.style}
                </span>
                <span className="px-2 py-0.5 bg-slate-100 text-slate-700 text-[11px] font-semibold rounded-md">
